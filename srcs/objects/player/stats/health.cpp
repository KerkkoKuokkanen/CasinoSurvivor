...
Health::Health()
{
	...
	num = new Number({8.28f, 5.0f}, std::to_string(health), 0.7f, {0.75f, 0.75f, 0.75f, 1.0f}, 200000.0f);
	std::string nn = std::to_string(health);
	float posX = 8.28f - (nn.length() - 1) * 0.7f;
	numPos = {posX, 5.0f};
	num->SetPosition({posX, 5.0f});
}
...
void Health::Hfunc1()
{
	if (numShakeTime <= 0.0f)
	{
		...
		num->SetColor(0.75f, 0.75f, 0.75f, 1.0f);
		return ;
	}
	if (numShakeMini >= 0.06f)
	{
		...
		if (cycle2)
		{
			num->SetColor(1.0f, 0.7f, 0.0f, 1.0f);
			cycle2 = false;
		}
		else
		{
...
void Health::Hfunc2()
{
	...
	if (numberAddTime >= numAddTime)
	{
		...
		float posX = 8.28f - (nn.length() - 1) * 0.7f;
		num = new Number({posX, 5.0f}, nn, 0.7f, {0.75f, 0.75f, 0.75f, 1.0f}, 220.0f);
		numPos = {posX, 5.0f};
		numberAddTime = 0.0f;
	}
