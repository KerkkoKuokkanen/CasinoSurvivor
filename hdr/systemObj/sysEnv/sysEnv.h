...
class SysEnv
{
	private:
		...
		std::vector<std::tuple<uint64_t, uint32_t, bool>> compDeleting = {};
	...
	public:
		...
		void ComponentRemove(uint64_t key, uint32_t id, bool destructed = false) {compDeleting.push_back({key, id, destructed});};
